...
function ENT:OnTakeDamage(damageInfo)
	...
	if IsValid(player) and player:IsPlayer() then
		...
		if (damageInfo:IsDamageType(128) and damageInfo:GetDamage() >= 15) or activeWeapon.isPickaxe or (player:GetSubfaction() == "Gotnarh" and IsValid(player:GetActiveWeapon()) and player:GetActiveWeapon():GetClass() == "begotten_fists") then
